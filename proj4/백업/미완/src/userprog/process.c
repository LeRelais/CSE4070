...
process_execute (const char *file_name) 
{
  ...
  char fn_copy2[100], *tmp, *savePtr;
  strlcpy(fn_copy2, file_name, strlen(file_name)+1);

  tmp = strtok_r(fn_copy2, " ", &savePtr);
  ...
  sema_down(&thread_current()->loadThread);
  if (tid == TID_ERROR)
...
start_process (void *file_name_)
{
  ...
  create_table(&thread_current()->virtualMemoryTable);

  /* If load failed, quit. */
  palloc_free_page (file_name);
  sema_up(&thread_current()->parent->loadThread);
  if (!success) {
    thread_current()->waitStatus = 1;
  }
...
process_exit (void)
{
  ...
  uint32_t *pd;
  destroy_table(&cur->virtualMemoryTable);
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      if (kpage == NULL){
        ...
        free_frame(&thread_current()->pagedir);
      ...
      allocate_frame(upage, kpage, writable, PAL_USER, thread_current()->pagedir);
